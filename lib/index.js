...
class ServerlessPlugin {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      'after:deploy:function:packageFunction': () => BbPromise.bind(this).then(() => {
        if (this.options.function) {
          this.createCustomFunctionPackage(this.options.function)
        }
        else {
          new Error('Function name must be set');
        }
      })
    }
  }
